...
- [x] Explain the difference between `Relational Databases` and `SQL`.
  ...
  SQL - Structured Query Language is the standard API of relational databases, Relational Database - set of formally described tables with each table record uniquely described by it's "primary key" value.

- [x] Why do tables need a `primary key`?

  Primary key is there to uniquely identify each record inside the table. Without it we wouldn't know which records to return to user.
